export function toGrid(serialized) {
  ...
  const grid = [];
  for (let r = 0; r < 9; r++) grid.push(values.slice(r * 9, r * 9 + 9));
  return grid;
}

export function isValidPlacement(grid, row, col, value) {
  if (value === 0) return true;
  for (let c = 0; c < 9; c++) if (c !== col && grid[row][c] === value) return false;
  for (let r = 0; r < 9; r++) if (r !== row && grid[r][col] === value) return false;
...
export function getConflicts(grid) {
  ...
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      const v = grid[r][c];
      if (v !== 0 && !isValidPlacement(grid, r, c, v)) conflicts[r][c] = true;
    }
  }
  return conflicts;
}
...
export function isCompleteAndValid(grid) {
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      const v = grid[r][c];
      if (v === 0 || !isValidPlacement(grid, r, c, v)) return false;
    }
  }
  return true;
}
